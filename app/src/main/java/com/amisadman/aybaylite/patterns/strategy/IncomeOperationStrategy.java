...
public class IncomeOperationStrategy implements DataOperationStrategy {
    ...
    // used for IncomeOperationStrategyTest
    ...
    public List<Transaction> loadTransactions(Context context) {
        ...
        ArrayList<HashMap<String, String>> data = db.getAllIncome();
        ...
        for (HashMap<String, String> map : data) {
            try {
                ...
                transactions.add(TransactionFactory
                        .createTransaction("INCOME", id, amount, reason, time));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public boolean deleteTransaction(Context context, String id) {
        return getDbHelper(context).deleteIncome(id);
    }
    ...
    public void addTransaction(Context context, Transaction transaction) {
        ...
        if (amount >= minimum && amount <= maximum) {
            getDbHelper(context).addIncome(transaction.getAmount(), transaction.getReason(), transaction.getTime());
        } else {
    ...
    public void updateTransaction(Context context, Transaction transaction) {
        double amount = transaction.getAmount();
        if (amount >= minimum && amount <= maximum) {
            getDbHelper(context).updateIncome(transaction.getId(), transaction.getAmount(), transaction.getReason(),
                    transaction.getTime());
        } else {
            throw new IllegalArgumentException("Amount is outside valid range");
        }
